clear
capture log close
gl logs "[your log file directory]"
gl do "[your code directory]"
gl censdata "[your raw census data directory]"
gl geoxwalk "[your crosswalk directory]"
gl data "[your central data directory]"
gl outdata "$data\pop"
gl  dosource "$do\1a_readpums_cens_v3.do"

*log using ${logs}\read_pums39_v2.log, replace
...
*Obs equal 1 if perwt is a positive num. Collapsed later and used for std errors
gen obs = (perwt > 0 & perwt < .)
keep if hispan==0 & inrange(race,1,2) & inrange(age,25,84)
drop if inlist(statefip,13,22,36,40,44,46,53)==1

summ

gen int age_lt=5*floor(age/5)

compress
rename birthyr birthyear
gen pop_ghs_1=inrange(educ,7,11)*perwt
gen obs_ghs_1=inrange(educ,7,11)*obs
gen pop_ghs_0=inrange(educ,0,6)*perwt
gen obs_ghs_0=inrange(educ,0,6)*obs
gen pop_coll_1=inrange(educ,10,11)*perwt
gen obs_coll_1=inrange(educ,10,11)*obs
gen pop_coll_0=inrange(educ,0,9)*perwt
gen obs_coll_0=inrange(educ,0,9)*obs

recode educ (0/1=1), gen(edcommon89)
merge m:1 sex race birthyear edcommon89 using $data\edrank_xw89
gen pop_hi75_1=(1-p_lo25)*perwt
gen obs_hi75_1=(1-p_lo25)*obs
gen pop_hi75_0=p_lo25*perwt
gen obs_hi75_0=p_lo25*obs

rename perwt pop_tot
rename obs obs_tot


collapse (sum) pop* obs*, by(year statefip puma sex race age_lt )
compress
gen diff_lev=pop_tot-pop_ghs_1-pop_ghs_0
gen diff_rank=pop_tot-pop_hi75_1-pop_hi75_0
summ diff*, d
notes : Created on ${S_DATE} by $dosource 
save "$outdata\decennial_1990_2000_wide.dta", replace
