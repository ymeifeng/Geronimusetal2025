...
PROJECT:	Create Survival and YPLL tables by age, race, sex, education, and CZA
			for the alled version
			
CREATED BY:	Vincent Pancini
UPDATED ON: 2023-06-15

*******************************************************************************/
global	logs	"$home\logs\all_ed_version"
global  out	"[analytic file directory]"
********************************************************************************
...
bys  cza noedst sex race: egen flag_pop = max(lowpop)
...
forvalues age = 25(5)80 {
	bys year  cza noedst race sex (age_lt): replace agecheck = `age' if age_lt[`check'] != `age' & agecheck == 0
	local check = `check' + 1
}
tab agecheck
	mdesc Mx_deaths_0 if cza == "10302" // 816 missings
...
*/
bys  cza noedst sex race: egen flag_missing_age = max(agecheck)
drop if agecheck
...
quietly gen qx_deaths_0 = min(1,(5 * Mx_deaths_0) / (1 + 2.5 * Mx_deaths_0))
...
forvalues age = 30(5)85 {
	quietly bys year  cza noedst race sex (age_lt): replace lx_ = lx_[_n-1] * (1 - qx_deaths_0[_n-1]) if age_lt == `age'
...
quietly bys year  cza noedst race sex (age_lt): gen dx_deaths_0 = abs(lx_[_n+1] - lx_)
/*
foreach cod of global causelist {
	capture noisily {
...
quietly bys year  cza noedst race sex (age_lt): gen Lx_ = 2.5 * (lx_ + lx_[_n + 1])
...
foreach cod of global causelist {
	capture noisily {
		quietly bys year  cza noedst race sex (age_lt): ///
...
bys year statefips cza sex race educ (age_lt): ereplace fxflg = max(fxflg)
bys statefips cza sex race educ (age_lt): egen flag_fx = max(fxflg)
...
summ ytest, det
 // Looks great!
*/
order year  cza noedst race sex age* Px* deaths* Mx* qx* lx_* dx* Lx_  yll* 
keep year  cza noedst race sex age* Px* deaths* Mx* qx* lx_* dx* Lx_  yll*

sort year  cza noedst race sex age_lt


save $out\detailed_deathrates_nostcz_alled_res.dta, replace
