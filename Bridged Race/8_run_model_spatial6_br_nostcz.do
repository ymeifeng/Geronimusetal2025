...
PURPOSE: Runs core models of mortality on bartik measures
         ...
         using bridged race population denominators
...
global analysisfile "$out\hist_race_income_bartik_mortchg_cznost_br.dta"
global logfile	"$logs\`mdir'\run_spatial_br_6_${sample_restriction}_$S_DATE.log"
global dosource	"$do\8_run_model_spatial6_br_nostcz.do"

global vintage "[date program run]"
...
putexcel set "${output}\spatial_res_${measure}_br_${vintage}_`race_group'", modify sheet("`race_group'_`age_group' ${sample_restriction}" replace)
...
putexcel J4        =     matrix(bBARTIK_male_`race_group'_1) ///
		 ...
		 , colnames nformat(number_d2) right
...
save "$data\fullsample_br", replace

use `final'
merge m:1  czone sex race using $data\fullsample_cznost
keep if _merge==3
drop _merge
