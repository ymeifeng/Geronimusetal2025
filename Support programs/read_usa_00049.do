global 	censdata "[raw census data directory]"
...
quietly infix                ///
  ...
  byte    empstatd  96-97    ///
  int     ind1990   98-100   ///
  byte    wkswork1  101-102  ///
  byte    uhrswork  103-104  ///
  using "$rawdata\usa_00049.dat"
...
gen birthyear=1980-age-1

gen byte school=(higraded-int(higraded/10)*10==2)
keep if (school~=1 & inrange(age,16,64)==1)
gen byte ind_2dig=floor(ind1990/10)
replace ind_2dig=39 if ind_2dig==38
drop if ind_2dig==0
gen edcommon89=educ
replace edcommon89=1 if edcommon89==0
gen byte ed_ghs=inrange(edcommon89,7,11)
gen byte r_hisp=1*(race==1 & hispan==0)+2*(race==2 & hispan==0) +3*(inrange(hispan,1,4))
replace r_hisp=4 if r_hisp==0
gen totpop_all=perwt
gen totemp_all=perwt*(empstat==1 & inrange(ind1990,0,940)==1)
gen tothrs_all=totemp_all*wkswork1*uhrswork
preserve
gen totpop_r_nhw=totpop_all*(r_hisp==1)
gen totpop_r_nhb=totpop_all*(r_hisp==2)
gen totemp_r_nhw=totemp_all*(r_hisp==1)
gen totemp_r_nhb=totemp_all*(r_hisp==2)
gen tothrs_r_nhw=tothrs_all*(r_hisp==1)
gen tothrs_r_nhb=tothrs_all*(r_hisp==2)
collapse (sum) totpop* totemp* tothrs*, by(ind_2dig statefip cntygp98)
save "$data/1980_indagg_byrace.dta", replace

restore
merge m:1 birthyear edcommon89 using "$data/edrank_bartik.dta"

gen totpop_ed_ghs=totpop_all*ed_ghs
gen totpop_ed_lehs=totpop_all*(1-ed_ghs)
gen totpop_ed_lo25=totpop_all*p_lo25
gen totpop_ed_hi75=totpop_all*(1-p_lo25)	
gen totemp_ed_ghs=totemp_all*ed_ghs
gen totemp_ed_lehs=totemp_all*(1-ed_ghs)
gen totemp_ed_lo25=totemp_all*p_lo25
gen totemp_ed_hi75=totemp_all*(1-p_lo25)	
gen tothrs_ed_ghs=tothrs_all*ed_ghs
gen tothrs_ed_lehs=tothrs_all*(1-ed_ghs)
gen tothrs_ed_lo25=tothrs_all*p_lo25
gen tothrs_ed_hi75=tothrs_all*(1-p_lo25)	

collapse (sum) totpop* totemp* tothrs*, by(ind_2dig statefip cntygp98)
save "$data/1980_indagg_byed.dta", replace
